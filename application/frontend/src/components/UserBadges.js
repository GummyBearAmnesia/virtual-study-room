...
const UserBadges = () => {
    ...
    const fetchBadges = async () => {
      ...
      try {
        const response = await axios.get(
          "https://virtual-study-room-unly.onrender.com/api/analytics/", // Endpoint for fetching analytics
          {
            headers: {
  ...
  const getBadgeEarnedDate = (rewardNumber) => {
    const badge = userBadges.find(
      (badge) => badge.reward_number === rewardNumber
    );
