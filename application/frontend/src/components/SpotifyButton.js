...
export default class SpotifyButton extends Component {
  ...
  authenticateSpotify() {
    ...
    fetch("https://virtual-study-room-unly.onrender.com/api/is-authenticated", {
      credentials: "include",
    })
      ...
      .then((data) => {
        ...
        if (!data.status && !this.state.redirecting) {
          ...
          fetch(
            "https://virtual-study-room-unly.onrender.com/api/get-auth-url",
            {
              credentials: "include",
            }
          )
  ...
  getCurrentSong() {
    fetch("https://virtual-study-room-unly.onrender.com/api/current-playing", {
      credentials: "include",
    })
      .then((response) => {
        if (!response.ok) {
